...
func (r *repository) RecapDmo(year string, iupopkId int) (ReportDmoOutput, error) {
	...
	queryFilter := fmt.Sprintf("transactions.seller_id = %v AND transactions.transaction_type = 'DN' AND dmos.period LIKE '%%%v' AND transactions.dmo_id IS NOT NULL AND grouping_vessel_dns.sales_system != 'Vessel'", iupopkId, year)
