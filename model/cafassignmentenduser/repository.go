...
func (r *repository) DetailCafAssignment(id int, iupopkId int) (DetailCafAssignment, error) {
	...
	if cafAssignment.LetterNumber != "" {
		...
		for _, value := range listAssignment {
			...
			query := fmt.Sprintf("transactions.transaction_type = '%s' AND transactions.seller_id = %v AND transactions.is_not_claim = false AND transactions.shipping_date >= '%s' AND transactions.shipping_date <= '%s' AND company.company_name = '%s' AND transactions.dmo_id IS NOT NULL AND transactions.report_dmo_id IS NOT NULL AND (gvd.sales_system != 'Vessel' or transactions.grouping_vessel_dn_id IS NULL) ", "DN", iupopkId, shippingDateFrom, shippingDateTo, value.EndUserString)

			errTrRealization := r.db.Table("transactions").Select("SUM(transactions.quantity) as realization_quantity, AVG(transactions.quality_calories_ar) as realization_average_calories ").Joins("left join companies company on company.id = transactions.dmo_buyer_id").Joins("left join grouping_vessel_dns gvd on gvd.id = transactions.grouping_vessel_dn_id").Where(query).Scan(&transactionRealization).Error
