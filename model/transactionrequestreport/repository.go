...
func (r *repository) PreviewTransactionReport(input masterreport.TransactionReportInput, iupopkId int) (TransactionRequestReportPreview, error) {
	...
	errFindDn := r.db.Table("transactions").Preload(clause.Associations).Preload("LoadingPort.Iupopk").Preload("UnloadingPort.PortLocation").Preload("DmoBuyer.IndustryType").Where("shipping_date >= ? and shipping_date <= ? and seller_id = ? and transaction_type = ?", input.DateFrom, input.DateTo, iupopkId, "DN").Find(&dnTransactions).Error

	if errFindDn != nil {
		return previewTransactions, errFindDn
	}

	errFindLn := r.db.Table("transactions").Preload(clause.Associations).Preload("LoadingPort.Iupopk").Preload("UnloadingPort.PortLocation").Preload("DmoBuyer.IndustryType").Where("shipping_date >= ? and shipping_date <= ? and seller_id = ? and transaction_type = ?", input.DateFrom, input.DateTo, iupopkId, "LN").Find(&lnTransactions).Error
...
func (r *repository) DetailTransactionReport(id int, iupopkId int) (TransactionRequestReportDetail, error) {
	...
	errFindDn := r.db.Table("transactions").Preload(clause.Associations).Preload("LoadingPort.Iupopk").Preload("UnloadingPort.PortLocation").Preload("DmoBuyer.IndustryType").Where("shipping_date >= ? and shipping_date <= ? and seller_id = ? and transaction_type = ?", transactionReqDetail.DateFrom, transactionReqDetail.DateTo, iupopkId, "DN").Find(&transactionsReportDn).Error
	...
	errFindLn := r.db.Table("transactions").Preload(clause.Associations).Preload("LoadingPort.Iupopk").Preload("UnloadingPort.PortLocation").Preload("DmoBuyer.IndustryType").Where("shipping_date >= ? and shipping_date <= ? and seller_id = ? and transaction_type = ?", transactionReqDetail.DateFrom, transactionReqDetail.DateTo, iupopkId, "LN").Find(&transactionsReportLn).Error
