...
func (r *repository) ListGroupingVesselLnWithPeriod(month string, year int, iupopkId int) ([]GroupingVesselLn, error) {
	...
	errFind := r.db.Preload(clause.Associations).Order("peb_register_date asc").Where("peb_register_date >= ? AND peb_register_date <= ? AND insw_id is NULL AND iupopk_id = ?", firstOfMonth, lastOfMonth, iupopkId).Find(&listGroupingVesselLn).Error
