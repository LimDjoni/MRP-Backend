...
func (r *repository) TraderListWithDmoId(idDmo int) ([]trader.Trader, trader.Trader, error) {
	...
	for _, v := range traderDmoList {
		...
		findTempErr := r.db.Preload(clause.Associations).Preload("Company.IndustryType.CategoryIndustryType").Where("id = ?", v.TraderId).First(&traderTemp).Error
...
func (r *repository) GetTraderEndUserDmo(idDmo int) (trader.Trader, error) {
	...
	errFind := r.db.Preload(clause.Associations).Preload("Trader.Company.IndustryType.CategoryIndustryType").Where("dmo_id = ? AND is_end_user = ?", idDmo, true).First(&endUserDmo).Error
